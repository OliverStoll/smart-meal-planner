...
def test_scrape_all_products(supermarket_name):
    ...
    assert (
        len(results) > 0
    ), f"Expected to find product data for {supermarket_name}, but found none."
...
def test_get_all_product_links(supermarket_name):
    ...
    assert (
        len(links) > 0
    ), f"Expected to find product links for {supermarket_name}, but found none."
...
def test_scrape_product(supermarket_product_url):
    ...
    assert (
        len(data) > 0
    ), f"Expected to find product data for {supermarket_product_url}, but found none."
    driver.close()
