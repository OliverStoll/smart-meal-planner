...
def test_filtered_recipes(user_settings, cleaned_recipes):
    ...
    assert len(recipes) == len(
        cleaned_recipes
    ), "No filtering should occur with default user settings"
...
def test_sample_recipes(user_settings, cleaned_recipes):
    ...
    samples = sample_recipes(
        num_recipes=num_samples, user_settings=user_settings, recipes=cleaned_recipes
    )
    assert len(samples) == num_samples, "Sampling fn should sample given amount"


def test_recipe_titles_by_id(cleaned_recipes):
    titles = recipe_titles_by_id(
        recipes=cleaned_recipes, recipe_ids=[cleaned_recipes["id"][0]]
    )
