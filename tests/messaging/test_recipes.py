...
def recipe_manager():
    ...
    def fake_init(self, data_path=None, ingredients_df_path=None, pdf_paths=None):
        ...
        self.ingredients_df = sample_ingredients.copy()
...
class TestSampleFittingRecipes:
    ...
    def test_uses_provided_recipes_dataframe(self, recipe_manager):
        ...
        result = recipe_manager.sample_fitting_recipes(
            num_recipes=2, user_settings=user_settings, recipes=custom_df
        )
        assert len(result) <= 2
    ...
    def test_returns_all_when_fewer_than_requested(self, recipe_manager):
        ...
        result = recipe_manager.sample_fitting_recipes(
            num_recipes=5, user_settings=user_settings, recipes=custom_df
        )
        assert len(result) == 1
