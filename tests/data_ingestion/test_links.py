...
class TestHelloFreshLinkCrawler:
    ...
    def test_get_recipe_links_of_category(self, category_path):
        ...
        links = self.crawler.get_recipes_links_of_category(
    ...
    def test_get_all_recipes_links(self, category_paths):
        ...
        assert isinstance(
            recipes_links, pd.DataFrame
        ), "Expected a DataFrame of recipe links"
        assert len(recipes_links) > 50, "Expected at least 50 recipe links"
        assert (
            "link" in recipes_links.columns
        ), "Expected 'link' column in the DataFrame"
        assert (
            "category" in recipes_links.columns
    ...
    def test_assure_recipe_links_from_db(self, single_category_path):
        ...
        assert isinstance(
            recipe_links, pd.DataFrame
        ), "Expected a DataFrame of recipe links from the database"
        assert (
            len(recipe_links) > 10
        ), "Expected at least 10 recipe links in the database"
    ...
    def test_assure_recipe_links_crawled(self, single_category_path):
        with patch.object(
            ...
            with patch.object(
                ...
                assured_links = self.crawler.assure_recipe_links(
                    use_stored=True, save_to_db=False
                )
        assert isinstance(
            assured_links, pd.DataFrame
        ), "Expected a DataFrame of assured recipe links"
