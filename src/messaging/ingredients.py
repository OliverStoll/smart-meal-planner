...
def clean_ingredient_data(
    ingredients: pd.DataFrame, filter_home_ingredients: bool
) -> pd.DataFrame:
    ...
    if filter_home_ingredients:
        ...
        ingredients = ingredients[
            ~ingredients["name"].str.contains(pattern, case=False)
        ]
...
def sort_ingredients_data(
    ingredients: pd.DataFrame, sorting: Literal["category", "amount"]
) -> pd.DataFrame:
    ...
    if sorting == "category":
        ...
        ingredients = ingredients.sort_values(
            by=["is_stueck", "quantity"], ascending=False
        )
    ingredients.drop(columns="is_stueck", inplace=True)
    return ingredients


def clean_ingredients_quantity_data(
    ingredients: pd.DataFrame, num_portions: int
) -> pd.DataFrame:
...
def _generate_ingredients_shopping_list_text(
    ingredients: pd.DataFrame, min_digits: int = 3
) -> str:
