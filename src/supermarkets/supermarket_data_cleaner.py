...
def clean_sellers(data_path: str = "data/supermarkets/product_data.csv") -> pd.DataFrame:
    ...
    data['sellers_partially'] = data['sellers'].apply(
        lambda x: [seller.split(' (gelegentlich')[0] for seller in x if
                   "(gelegentlich im Sortiment)" in seller])
    data['sellers_always'] = data['sellers'].apply(
        lambda x: [seller for seller in x if "(gelegentlich im Sortiment)" not in seller])
    data['sellers_always'] = data['sellers'].apply(
        lambda x: [SUPERMARKET_FRIENDLY_NAMES.get(seller, seller) for seller in x if
                   seller in SUPERMARKET_FRIENDLY_NAMES])
