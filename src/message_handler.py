...
class MessageHandler:
    ...
    def send_meals_message(self, chat_id: int, num_meals: int, message_to_edit_id: int | None =None, recipes_to_send=None):
        """ 
        ...
        Args:
            ...
            message_to_edit_id: The message ID of the previous message (optional).
            recipes_to_send: The recipes to send (optional).
        """
        # todo: remove all sent recipes
        user_settings = self.options_handler.get_user_settings(chat_id)

        if recipes_to_send is None:
            recipes_to_send = self.meal_manager.get_recipes_filtered_by_user_settings(
                num_meals, user_settings
            )
        self.last_recipes_df[str(chat_id)] = recipes_to_send

        ingredient_shopping_list = self.meal_manager.get_ingredients_shopping_list(
            recipes_df=recipes_to_send, num_portions=user_settings.portions
        )

        title_response = self._get_title_response(num_meals, user_settings.meal_type, user_settings.portions)
        message_data = {
            'chat_id': chat_id,
        ...
        if message_to_edit_id:
            ingredient_msg = self.bot.edit_message_text(message_id=message_to_edit_id, **message_data)
        else:
    ...
    def replace_meal(self, call, idx, ingredient_msg_id):
        ...
        if self.last_recipes_df.get(str(chat_id), None) is None:
            return
        chat_id = call.message.chat.id
        portions, meal_type, max_duration, cal_min = self.options_handler.get_user_settings(chat_id)
        ...
        self.send_meals_message(
            ...
            message_to_edit_id=ingredient_msg_id,
